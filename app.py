...
UI_FONT_NAME = "Righteous"
UI_FONT_FILE = "Righteous-Regular.ttf"
CYRILLIC_FONT_NAME = "Trebuchet MS"
AUTHOR_GITHUB_URL = "https://github.com/iglakovmaks"

LATIN_OR_DIGIT_RE = re.compile(r"[A-Za-z0-9]")


def _uses_latin_or_digits(text: str) -> bool:
    return bool(LATIN_OR_DIGIT_RE.search(text))


def _resolve_resource_path(filename: str) -> Optional[Path]:
...
def _register_private_font(font_path: Path) -> bool:
    try:
        ...
        if sys.platform == "darwin":
            ...
            raw_path = os.fsencode(font_path)
            ...
            try:
                ...
                return bool(core_text.CTFontManagerRegisterFontsForURL(url, kCTFontManagerScopeProcess, None))
            finally:
...
class RoundedHintCard(tk.Canvas):
    def __init__(
        ...
        *,
        ...
        latin_body_font: tuple[str, int] | tuple[str, int, str],
        min_height: int = 150,
        radius: int = 14,
    ) -> None:
        super().__init__(
        ...
        self._latin_body_font = latin_body_font
        self._min_height = min_height
        self._radius = radius
        self._trace_id = self._text_var.trace_add("write", self._on_text_change)
        self._ru_body_metrics = tkfont.Font(font=self._body_font)
        self._latin_body_metrics = tkfont.Font(font=self._latin_body_font)
        ...
        self._body_text.tag_configure("ru", font=self._body_font, foreground=COLOR_MILK)
        self._body_text.tag_configure("latin", font=self._latin_body_font, foreground=COLOR_MILK)
    ...
    def _render_body_text(self) -> int:
        content = self._text_var.get()
        ...
        for part in parts:
            if not part:
                continue
            if part.isspace():
                self._body_text.insert(tk.END, part, ("ru",))
                continue
            tag = "latin" if _uses_latin_or_digits(part) else "ru"
            self._body_text.insert(tk.END, part, (tag,))

        self._body_text.configure(state=tk.DISABLED)
        self._body_text.update_idletasks()

        try:
            display_lines = int(self._body_text.count("1.0", "end-1c", "displaylines")[0])
        except Exception:
            display_lines = 1
        if display_lines < 1:
            display_lines = 1
        ...
        line_height = max(
            self._ru_body_metrics.metrics("linespace"),
            self._latin_body_metrics.metrics("linespace"),
        )
        return display_lines * line_height + 4
    ...
    def _redraw(self) -> None:
        ...
        self.create_window(
            ...
            44,
            anchor="nw",
            window=self._body_text,
            width=max(40, w - 32),
            height=max(24, h - 60),
        )

        body_height = self._render_body_text()
        needed_h = max(self._min_height, 44 + body_height + 16)
...
class ColorModeSwitch(tk.Frame):
    def __init__(
        ...
        *,
        text: str,
        label_font: tuple[str, int] | tuple[str, int, str],
...
class ChessHelperApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.ui_latin_font_family, self.ui_cyrillic_font_family = self._setup_ui_fonts()
    ...
    def _latin_font(self, size: int, weight: Optional[str] = None) -> tuple[str, int] | tuple[str, int, str]:
        if weight:
            return (self.ui_latin_font_family, size, weight)
        return (self.ui_latin_font_family, size)

    def _ru_font(self, size: int, weight: Optional[str] = None) -> tuple[str, int] | tuple[str, int, str]:
        if weight:
            return (self.ui_cyrillic_font_family, size, weight)
        return (self.ui_cyrillic_font_family, size)

    def _setup_ui_fonts(self) -> tuple[str, str]:
        default_font = tkfont.nametofont("TkDefaultFont")
        fallback_family = default_font.cget("family")
        available_families = set(tkfont.families(self.root))

        cyrillic_family = CYRILLIC_FONT_NAME if CYRILLIC_FONT_NAME in available_families else fallback_family
        latin_family = cyrillic_family
        font_path = _resolve_resource_path(UI_FONT_FILE)
        if font_path:
            _register_private_font(font_path)

        available_families = set(tkfont.families(self.root))
        if UI_FONT_NAME in available_families:
            latin_family = UI_FONT_NAME
        ...
        for named_font in (
            ...
            try:
                tkfont.nametofont(named_font).configure(family=cyrillic_family)
            except tk.TclError:
                pass
    ...
    def _configure_theme(self) -> None:
        ...
        style.configure(
            ...
            background=COLOR_PANEL,
            foreground=COLOR_NAVY,
            font=self._ru_font(16),
        )
        style.configure(
            "Panel.TLabel",
            background=COLOR_PANEL,
            foreground=COLOR_PANEL_TEXT,
            font=self._ru_font(14),
        )
        style.configure(
            "Status.TLabel",
            background=COLOR_PANEL,
            foreground="#7B808F",
            font=self._ru_font(11),
        )

        style.configure(
            "App.TButton",
            font=self._ru_font(13, "bold"),
            padding=(14, 10),
    ...
    def _build_ui(self) -> None:
        ...
        self.author_label = tk.Label(
            ...
            bg=COLOR_PANEL,
            fg=COLOR_PANEL_TEXT,
            font=self._latin_font(14),
            bd=0,
            highlightthickness=0,
            cursor="arrow",
        )
        self.author_label.pack(side=tk.LEFT)
        self.author_label.bind("<Button-1>", self._open_author_link)
        ...
        self.hint_card = RoundedHintCard(
            ...
            title_font=self._ru_font(15, "bold"),
            body_font=self._ru_font(18),
            latin_body_font=self._latin_font(18),
            min_height=148,
            radius=12,
        )
    ...
    def _open_author_link(self, _: tk.Event | None = None) -> None:
        try:
            webbrowser.open_new_tab(AUTHOR_GITHUB_URL)
        except Exception:
            pass
    ...
    def _resolve_engine_path(self) -> str:
        ...
        if getattr(sys, "frozen", False):
            ...
            if exe_dir.name == "MacOS":
                ...
                dist_dir = app_bundle_dir.parent
                sibling_dir = dist_dir / exe_path.stem
                search_dirs.extend([contents_dir, app_bundle_dir, sibling_dir])

            meipass = getattr(sys, "_MEIPASS", None)
            if meipass:
                meipass_dir = Path(meipass)
                search_dirs.append(meipass_dir)
                search_dirs.append(meipass_dir.parent)
